...
void StartSensorTask(void *argument)
{
  ...
  #define SOIL_WET_VALUE 1019  // Your measured value from water
  #define SOIL_DRY_VALUE 2687  // Your measured value from dry air

  // --- How many times to sample the ADC to reduce noise ---
  ...
  for(;;)
  {
    ...
    // --- Read Soil Moisture with Oversampling and Averaging ---
    ...
    // Clamp the averaged raw value to the calibrated range
    ...
    // Map the stable raw value to a percentage (0-100)
    ...
    DHT11_StatusTypeDef dht_status = DHT11_Read_Data(&temp_int, &hum_int);
    ...
    {
        // If the read fails, set values to 0
